...
getJson(masterUrl, (err: any, json: any) => {
  if (err) {
    throw err
  }
  ...
  processFile('video', videoBaseUrl, videoData.init_segment, videoData.segments, json.clip_id + '.m4v', (err: any) => {
    if (err) {
      throw err
    }
    
    processFile('audio', audioBaseUrl, audioData.init_segment, audioData.segments, json.clip_id + '.m4a', (err: any) => {
      if (err) {
        throw err
      }

      console.log(json.clip_id)
    })
  })
})

function processFile(type: string, baseUrl: string, initData: string, segments: any, filename: string, cb: any) {
  if (fs.existsSync(filename)) {
    //console.log(`${type} already exists`)
    return cb()
  }
  
  const segmentsUrl = segments.map((seg: any) => baseUrl + seg.url)
  ...
  combineSegments(type, 0, segmentsUrl, output, (err: any) => {
    if (err) {
      return cb(err)
    }
...
function combineSegments(type: string, i: number, segmentsUrl: string, output: any, cb: any) {
